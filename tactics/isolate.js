        ...
        },
        {
            ...
            "implementationStrategies": [
                ...
                },
                {
                    ...
                    "howTo": "<h5>Concept:</h5><p>Continuously compute trip metrics from validators/schemas (<code>consensus_score</code>, <code>evidence_coverage</code>, <code>contradiction_rate</code>, <code>schema_error_rate</code>, <code>unknown_tool_calls</code>). On trip, switch to {<code>read_only</code>, <code>dry_run</code>, <code>safe_tools_only</code>, <code>HITL_required</code>}; quarantine KB writes; rollback recent writes; tighten rate-limits.</p><h5>Express middleware (Node) breaker:</h5><pre><code class=\"language-javascript\">// breaker.js\nexport function breaker(req, res, next){\n  const m = req.app.locals.metrics;\n  const tripped = (m.consensus < 0.6) || (m.evidence_coverage < 0.5) || (m.contradiction_rate > 0.2) || (m.schema_error_rate > 0.1) || (m.unknown_tool_calls > 0);\n  if (!tripped) return next();\n  req.app.locals.mode = \"safe_tools_only\"; // degrade\n  res.set(\"X-Agent-Mode\", req.app.locals.mode);\n  next();\n}\n</code></pre><h5>Python FastAPI toggle & quarantine hook:</h5><pre><code class=\"language-python\"># app.py\nfrom fastapi import FastAPI, Request\napp = FastAPI()\nstate = {\"mode\": \"normal\"}\n\n@app.middleware(\"http\")\nasync def breaker_mw(req: Request, call_next):\n    m = req.app.state.metrics\n    tripped = (m['consensus'] < 0.6) or (m['evidence_coverage'] < 0.5) or (m['contradiction_rate'] > 0.2)\n    if tripped:\n        state[\"mode\"] = \"read_only\"\n    resp = await call_next(req)\n    resp.headers[\"X-Agent-Mode\"] = state[\"mode\"]\n    return resp\n\n# write gate example\n@app.post(\"/kb/write\")\nasync def write_kb(item: dict):\n    if state[\"mode\"] in (\"read_only\", \"safe_tools_only\"):\n        return {\"status\": \"quarantined\"}\n    # ... normal write\n    return {\"status\": \"ok\"}\n</code></pre>"
                }
            ]
        },
        {
