export const modelTactic = {
    ...
    "techniques": [
        ...
        },
        {
            ...
            "description": "Establish a dual-dimensional governance framework for AI agent operations.<ul><li><strong>Dimension 1 — Agent Ceiling:</strong> each agent deployment is assigned a maximum autonomy level (L0–L5, per CSA Agentic Trust Framework) that caps its operational authority.</li><li><strong>Dimension 2 — Action Minimum:</strong> each tool or action is classified by blast radius into a minimum required level (per NIST per-action authorization).</li><li><strong>Enforcement:</strong> if agent_ceiling &lt; action_minimum → DENY. The agent cannot self-escalate.</li><li><strong>Human-in-the-loop (HITL) requirements by level:</strong><ul><li>L0–L1 require HITL for every action</li><li>L2 requires HITL for plan approval</li><li>L3 requires HITL only when boundaries are exceeded</li><li>L4–L5 operate with monitoring-only oversight</li></ul></li></ul>This ensures granular governance — the same agent may be authorized for low-risk reads but denied destructive operations — rather than blanket agent-level permissions.",
            "toolsOpenSource": [
            ...
            "implementationGuidance": [
                {
                    "implementation": "Implement dual-dimensional autonomy governance: agent ceiling (CSA L0–L5) × action minimum (per blast radius). Enforce with OPA.",
                    "howTo": "<h4>Overview</h4><p>This technique synthesizes three authoritative frameworks into one enforceable model:</p><ul><li><strong>CSA Agentic Trust Framework</strong> — agent autonomy levels (L0–L5), each with a mandated HITL posture</li><li><strong>NIST AI Agent Standards Initiative</strong> — per-action authorization, classified by blast radius</li><li><strong>OWASP Agentic AI Top 10 (ASI02/ASI03)</strong> — granular per-tool permissions with task-scoped credentials</li></ul><hr/><h4>Dimension 1 — Agent Ceiling (CSA L0–L5)</h4><p>Each agent deployment is assigned a <em>ceiling</em> — the maximum autonomy it is authorized for.</p><table><tr><th>Level</th><th>Name</th><th>Agent Behavior</th><th>Human-in-the-Loop</th></tr><tr><td><strong>L0</strong></td><td>No Autonomy</td><td>AI provides info/analysis only; cannot execute actions</td><td>Human performs all actions</td></tr><tr><td><strong>L1</strong></td><td>Assisted</td><td>AI executes actions one at a time</td><td><strong>HITL per action</strong> — human approves each before execution</td></tr><tr><td><strong>L2</strong></td><td>Supervised</td><td>AI executes a batch or plan autonomously within approved scope</td><td><strong>HITL per plan</strong> — human approves the plan, not each action</td></tr><tr><td><strong>L3</strong></td><td>Conditional</td><td>AI decides and acts autonomously within defined boundaries</td><td><strong>HITL at boundary</strong> — escalates only when boundary is exceeded</td></tr><tr><td><strong>L4</strong></td><td>High Autonomy</td><td>AI operates across broad scope with continuous monitoring</td><td><strong>No routine HITL</strong> — monitoring + anomaly alerts; executive sign-off at deployment</td></tr><tr><td><strong>L5</strong></td><td>Full Autonomy</td><td>Self-directed, can set own goals</td><td><strong>Strategic oversight only</strong> — not recommended for enterprise today</td></tr></table><hr/><h4>Dimension 2 — Action Minimum (by blast radius)</h4><p>Each tool or action is classified into a <em>minimum required level</em> based on its potential impact.</p><table><tr><th>Blast Radius</th><th>Min Level</th><th>HITL Posture</th></tr><tr><td>Read-only / retrieve</td><td>L1+</td><td>L0–L1 agents require approval; L2+ proceed autonomously</td></tr><tr><td>Data modification (reversible)</td><td>L2+</td><td>L0–L1 require per-action approval; L2 requires plan approval</td></tr><tr><td>External effect / boundary-edge</td><td>L3+</td><td>L0–L2 require approval; L3 proceeds if within boundary</td></tr><tr><td>Destructive / irreversible</td><td>L4+</td><td>L0–L3 <strong>always denied</strong>; only L4+ (with exec sign-off) can execute</td></tr></table><hr/><h4>Enforcement Rule</h4><pre><code>if agent_ceiling &lt; action_minimum → DENY\n\nThe agent cannot self-escalate its ceiling.</code></pre><p><strong>Key insight:</strong> Destructive actions require <em>high</em> autonomy authorization — because only the most vetted, exec-approved, continuously monitored agents should be trusted with irreversible operations.</p><hr/><h4>OPA/Rego Policy</h4><pre><code># File: policy/agent_autonomy.rego\npackage agent.autonomy\n\n# Numeric mapping for level comparison\nlevel_rank := {\n  \"L0\": 0, \"L1\": 1, \"L2\": 2,\n  \"L3\": 3, \"L4\": 4, \"L5\": 5,\n}\n\n# Agent ceiling: assigned at deployment (CSA)\nagent_ceiling := level_rank[input.agent.autonomy_level]\n\n# Action minimum: classified per tool by blast radius\naction_min := level_rank[input.tool_call.required_level]\n\n# Rule 1: Core enforcement — ceiling must meet or exceed action minimum\ndeny[msg] {\n  agent_ceiling &lt; action_min\n  msg := sprintf(\n    \"DENIED: %v requires %v — agent %v ceiling is %v.\",\n    [input.tool_call.tool_name, input.tool_call.required_level,\n     input.agent.id, input.agent.autonomy_level]\n  )\n}\n\n# Rule 2: HITL gate — L0-L1 agents need human approval for every write\ndeny[msg] {\n  agent_ceiling &lt;= 1\n  input.tool_call.write_access == true\n  not input.tool_call.hitl_approved == true\n  msg := sprintf(\n    \"HITL required: %v (write) needs approval for L0-L1 agent %v.\",\n    [input.tool_call.tool_name, input.agent.id]\n  )\n}\n\n# Rule 3: HITL gate — L2 agents need plan-level approval for write batches\ndeny[msg] {\n  agent_ceiling == 2\n  input.tool_call.write_access == true\n  not input.session.plan_approved == true\n  msg := sprintf(\n    \"Plan approval required: L2 agent %v — approve plan before batch execution.\",\n    [input.agent.id]\n  )\n}\n\n# Rule 4: L3 boundary check — escalate when outside defined boundaries\ndeny[msg] {\n  agent_ceiling == 3\n  input.tool_call.exceeds_boundary == true\n  not input.tool_call.hitl_approved == true\n  msg := sprintf(\n    \"Boundary exceeded: %v requires HITL escalation for L3 agent %v.\",\n    [input.tool_call.tool_name, input.agent.id]\n  )\n}\n</code></pre><hr/><h4>Tool Classification Policy (YAML)</h4><pre><code># File: policy/tool_classification.yaml\n# Classify each tool by blast radius → required autonomy level\ntools:\n  search_kb:\n    required_level: L1\n    write_access: false\n    description: Search knowledge base (read-only)\n  create_ticket:\n    required_level: L2\n    write_access: true\n    description: Create support ticket (reversible data modification)\n  send_email:\n    required_level: L3\n    write_access: true\n    description: Send email (boundary-edge, external effect)\n  execute_sql_select:\n    required_level: L2\n    write_access: false\n    description: Read-only SQL query\n  execute_sql_ddl:\n    required_level: L4\n    write_access: true\n    destructive: true\n    description: DDL operations (DROP, ALTER) — destructive, irreversible\n  rotate_credentials:\n    required_level: L4\n    write_access: true\n    destructive: true\n    description: Credential rotation — irreversible, broad impact\n</code></pre><hr/><h4>Deployment Steps</h4><ol><li><strong>Tag</strong> every agent deployment with an <code>autonomy_level</code> ceiling (L0–L5).</li><li><strong>Classify</strong> every tool/action with a <code>required_level</code> minimum based on blast radius.</li><li><strong>Deploy OPA</strong> as pre-action authorization gate — evaluate ceiling vs minimum before every tool call.</li><li><strong>Log</strong> all denials and escalations for audit and compliance.</li></ol>"
                }
            ]
        }
    ]
