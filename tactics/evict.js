            ...
            "defendsAgainst": [
                {
                    ...
                    "items": [
                        ...
                        "AML.T0017 Persistence (dismantling live footholds / manipulated state)"
                    ]
                },
                {
                    "framework": "MAESTRO",
            ...
            "implementationStrategies": [
                {
                    ...
                    "howTo": "<h5>Concept:</h5><p>In response to a confirmed compromise, invalidate active sessions to immediately evict any attacker using stolen session artifacts.</p><h5>Implement a Global Session Flush</h5><p>Clear the server-side session store (e.g., Redis) to kill existing sessions in one action.</p><pre><code># File: incident_response/flush_all_sessions.py\nimport redis\n\ndef flush_all_user_sessions():\n    \"\"\"Deletes all keys matching the session pattern using SCAN safely.\"\"\"\n    r = redis.Redis(host='localhost', port=6379, db=0)\n    cursor = '0'\n    total_deleted = 0\n    while True:\n        cursor, keys = r.scan(cursor=cursor, match=\"session:*\", count=1000)\n        if keys:\n            total_deleted += r.delete(*keys)\n        if cursor == 0:\n            break\n    print(f\"✅ Flushed {total_deleted} session keys.\")</code></pre><p><strong>Action:</strong> Maintain a tested admin script/playbook that can safely flush server-side session caches on demand.</p>"
                },
                {
                    "strategy": "Invalidate session tokens for AI agents and rotate signing keys.",
                    "howTo": "<h5>Concept:</h5><p>For stateless tokens (e.g., JWT), enforce revocation using a denylist keyed by the token's unique ID (`jti`). As a broader measure, rotate the token signing keys to invalidate all tokens signed with the old key, including any the attacker may have stolen but not yet used.</p><h5>Implement a JWT Revocation List</h5><pre><code># File: eviction_scripts/revoke_jwt.py\nimport redis\nimport jwt\nimport time\n\ndef revoke_jwt(token: str, secret: str, redis_client):\n    \"\"\"Adds a token's JTI to the revocation list with remaining TTL.\"\"\"\n    try:\n        payload = jwt.decode(token, secret, algorithms=[\"HS256\"], options={\"verify_exp\": False})\n        jti = payload.get('jti')\n        exp = payload.get('exp')\n        if not jti or not exp: return\n        remaining_ttl = max(0, exp - int(time.time()))\n        if remaining_ttl > 0:\n            redis_client.set(f\"jwt_revoked:{jti}\", \"revoked\", ex=remaining_ttl)\n    except jwt.PyJWTError as e:\n        print(f\"Invalid token: {e}\")</code></pre><p><strong>Action:</strong> Ensure issued JWTs include a unique `jti` and have a short expiry. Implement a revocation denylist and make rotating your JWT signing keys part of your incident response plan.</p>"
                },
                {
                    "strategy": "Clear persistent conversational histories/cached states for affected agents.",
                    "howTo": "<h5>Concept:</h5><p>Purge poisoned agent memory or state so new instances cannot re-load tainted context. This is a destructive action to remove malicious artifacts.</p><h5>Implement a Targeted State Purge Script</h5><pre><code># File: eviction_scripts/purge_agent_state.py\nimport redis\n\ndef purge_state_for_agents(agent_ids: list):\n    \"\"\"Deletes cache keys associated with given agent IDs.\"\"\" \n    r = redis.Redis()\n    keys_deleted = 0\n    for agent_id in agent_ids:\n        patterns = [f\"session:{agent_id}:*\", f\"chat_history:{agent_id}\", f\"agent_state:{agent_id}\"]\n        for pattern in patterns:\n            for key in r.scan_iter(pattern):\n                keys_deleted += r.delete(key)\n    print(f\"✅ Purged {keys_deleted} keys for {len(agent_ids)} agents.\")</code></pre><p><strong>Action:</strong> After terminating compromised processes (`AID-E-002`), purge all persisted state tied to those agent identities from your caching layers.</p>"
                },
                {
                    "strategy": "Disable execution footholds and malicious webhooks or tool registrations.",
                    "howTo": "<h5>Concept:</h5><p>An attacker may establish persistence by registering a malicious tool, a webhook pointing to their own server, or by leaving a malicious job in a queue. A full eviction requires dismantling these runtime footholds.</p><p><strong>Action:</strong> As part of an eviction playbook, perform the following: stop and cordon compromised agent pods/VMs (`AID-E-002`); review and disable any recently added or unrecognized tool registrations or API webhook configurations; purge any high-risk work queues (e.g., code execution queues); and temporarily freeze high-risk permission sets until they can be audited.</p>"
                }
            ]
        },
        {
